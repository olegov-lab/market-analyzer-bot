  ...
  try {
    ...
    let html = `
      <div class="card">
        ...
        <div class="price-large">$${fmtPrice(p)}</div>
      </div>`;

    if (ind) {
      html += `<div class="card">
        <div class="card-title">Технические индикаторы</div>`;
    ...
    if (pred) {
      ...
      html += `<div class="card">
        ...
        <div class="signal ${signalClass}">${signalEmoji} ${signalText}</div>
        <div class="conf-bar"><div class="conf-bar-fill ${confColor}" style="width:${confPct}%"></div></div>
        <div class="row"><span class="label">Уверенность</span><span class="value">${confPct}%</span></div>
      </div>`;
    }

    html += `<div class="card" style="font-size:11px;color:var(--hint);text-align:center;">
      ♻️ Обновление каждые 30с
    </div>`;
  ...
  try {
    ...
    if (pred) {
      ...
      html += `<div class="card">
        ...
        <div class="signal ${signalClass}">${signalEmoji} ${signal}</div>
        <div style="margin-top:8px;font-weight:600;">$${fmtPrice(priceMin)} – $${fmtPrice(priceMax)}</div>
        <div class="conf-bar"><div class="conf-bar-fill ${confColor}" style="width:${confPct}%"></div></div>
        <div class="row"><span class="label">Уверенность</span><span class="value">${confPct}%</span></div>
      </div>`;
      ...
      html += `<div class="card" style="font-size:11px;color:var(--hint);text-align:center;">
        ♻️ Прогноз — 1ч · On-chain — 6ч
      </div>`;
    } else {
      html = `<div class="card"><div class="card-title">Прогноз</div>
        ⏳ Собираем историю для прогноза (~48ч)</div>`;
    }
...
async function renderNews() {
  ...
  try {
    ...
    let html = `<div class="card">
      <div class="card-title">Пульс рынка</div>
      <div style="font-size:18px;font-weight:700;">${moodEmoji} ${moodText}</div>
      <div class="mood-row">
        <div class="mood-item bullish"><div class="count">${bullCount}</div><div class="mood-label">Бычьих</div></div>
        <div class="mood-item bearish"><div class="count">${bearCount}</div><div class="mood-label">Медвежьих</div></div>
        <div class="mood-item neutral"><div class="count">${neutralCount}</div><div class="mood-label">Нейтр.</div></div>
      </div>
      <div style="margin-top:8px;font-size:13px;">Тревога: ${worryLabel}</div>
    </div>`;
    ...
    for (const a of articles) {
      ...
      html += `<div class="news-item">
        <div class="news-title">${emoji} ${escapeHtml(a.title)}</div>
        <div class="news-meta"><a class="news-link" href="${escapeHtml(a.url)}" target="_blank">Читать${src}</a></div>
      </div>`;
    }
...
async function renderLearnList() {
  ...
  try {
    ...
    lessons = data;
    let html = `<div class="card"><div class="card-title">Азбука крипты</div>
      <p style="margin-bottom:12px;color:var(--hint);">10 коротких уроков для начинающих</p>`;
...
async function renderLesson(id) {
  ...
  try {
    ...
    let html = `<div class="card">
      <div class="card-title">Урок ${lesson.id}</div>
...
async function renderAlerts() {
  ...
  try {
      ...
      for (const sub of subs) {
        for (const at of sub.alert_types) {
          html += `<div class="alert-item">
            <span class="alert-type">${escapeHtml(at)}</span>
            <button class="btn-unsub" data-sub-id="${sub.id}" data-type="${escapeHtml(at)}">Отписаться</button>
          </div>`;
        }
      }
    }
    ...
    for (const [type, label] of [['rsi', 'RSI — перекупленность/перепроданность'], ['ma_cross', 'MA Cross — пересечение MA50 и MA200'], ['volume_spike', 'Volume Spike — аномальный объём']]) {
      html += `<button class="sub-btn" data-alert-type="${type}">+ ${escapeHtml(label)}</button>`;
    }
    html += `</div>`;
